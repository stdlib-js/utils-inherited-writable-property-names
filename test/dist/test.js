...
var inheritedWritablePropertyNames = require( './../../dist' ); // eslint-disable-line id-length
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof inheritedWritablePropertyNames, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if not provided a positive integer for the inheritance level', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			inheritedWritablePropertyNames( {}, value );
		};
	}
});

tape( 'the function returns an array of an object\'s inherited writable property names', function test( t ) {
	var expected;
	var actual;
	var obj;
	var idx;
	...
	expected = [ 'd', 'f' ];
	actual = inheritedWritablePropertyNames( obj );
...
tape( 'the function returns an array of an object\'s inherited writable property names (multiple inheritance)', function test( t ) {
	var expected;
	var actual;
	var obj;
	var idx;
...
tape( 'the function supports limiting the inheritance level', function test( t ) {
	...
	expected = [ 'foo' ];
	actual = inheritedWritablePropertyNames( obj, 1 );
...
tape( 'the function returns an empty array if provided `null` or `undefined`', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		actual = inheritedWritablePropertyNames( values[ i ] );
